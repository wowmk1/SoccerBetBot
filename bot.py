...
async def matches_command(interaction: discord.Interaction):
    ...
    # Post with competition headers
    for league_name, league_matches in league_dict.items():
        # Send competition separator
...
async def upcoming_command(interaction: discord.Interaction, user: discord.Member = None):
    ...
    for i in range(0, len(predictions), 20):
        ...
        for pred in chunk:
            ...
            home_team = format_team_name(pred['home_team'])
            away_team = format_team_name(pred['away_team'])
...
async def history_command(interaction: discord.Interaction, user: discord.Member = None, days: int = 7):
    ...
    for i in range(0, len(predictions), 20):
        ...
        for pred in chunk:
            ...
            embed.add_field(
                name=f"{result_emoji} {home_team} {pred['home_score']}-{pred['away_score']} {away_team}",
...
async def unpick_command(interaction: discord.Interaction, match_id: str):
    ...
    if delete_prediction(user_id, match_id):
        ...
        home_team = format_team_name(match_info['home_team'])
        away_team = format_team_name(match_info['away_team'])
        
        await interaction.response.send_message(
            f"Deleted your **{prediction.capitalize()}** prediction for {home_team} vs {away_team}",
            ephemeral=True
        )
    else:
...
async def compare_command(interaction: discord.Interaction, user: discord.Member):
    ...
    if head_to_head:
        ...
        for match in head_to_head:
            ...
            home_team = format_team_name(match['home_team'])
            away_team = format_team_name(match['away_team'])
            h2h_text.append(f"{home_team} {match['home_score']}-{match['away_score']} {away_team}: {result}")
...
bot.run(DISCORD_BOT_TOKEN)import os
import json
import aiohttp
import asyncio
import psycopg2
