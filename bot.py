...
def init_db():
    ...
    try:
        with db_connection() as conn:
            ...
            cur.execute("CREATE INDEX IF NOT EXISTS idx_predictions_composite ON predictions(user_id, match_id)")
            ...
            cur.execute("CREATE INDEX IF NOT EXISTS idx_weekly_stats_week ON weekly_stats(week_start)")
            cur.execute("CREATE INDEX IF NOT EXISTS idx_vote_data_match ON vote_data(match_id)")
...
def upsert_user(user_id, username):
    ...
    with db_connection() as conn:
        ...
        cur.execute("""
                    ...
                    VALUES (%s, %s, 0) ON CONFLICT (user_id) DO
...
def add_prediction(user_id, match_id, prediction):
    ...
    with db_connection() as conn:
        ...
        cur.execute("""
                    ...
                    VALUES (%s, %s, %s) ON CONFLICT (user_id, match_id) DO NOTHING
    ...
    with db_connection() as conn:
        ...
        if is_correct:
            cur.execute("""
                        ...
                        VALUES (%s, %s, 1, 1) ON CONFLICT (user_id, week_start) 
                DO
                        UPDATE SET correct = weekly_stats.correct + 1, total = weekly_stats.total + 1
                        """, (user_id, week_start))
        else:
            cur.execute("""
                        INSERT INTO weekly_stats (user_id, week_start, correct, total)
                        VALUES (%s, %s, 0, 1) ON CONFLICT (user_id, week_start) 
                DO
                        UPDATE SET total = weekly_stats.total + 1
                        """, (user_id, week_start))
...
def save_vote_message(match_id, msg_id):
    ...
    with db_connection() as conn:
        ...
        cur.execute("""
                    ...
                    VALUES (%s, %s) ON CONFLICT (match_id) DO
                    UPDATE SET votes_msg_id = EXCLUDED.votes_msg_id
                    """, (match_id, msg_id))
        conn.commit()
...
class VoteButton(Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        try:
            ...
            match_time = ensure_utc(match_info['match_time'])
...
async def start_health_server():
    ...
    try:
        ...
        # Don't crash the bot if health check fails
...
async def weekly_recap():
    try:
        ...
        # Only run on Mondays at 9 AM UTC
        ...
        for user_stat in last_week_stats:
            if user_stat['total'] >= 3:
                try:
                    ...
                    # Rank
                    rank = next((i + 1 for i, u in enumerate(last_week_stats) if u['user_id'] == user_stat['user_id']),
                                None)
                    if rank:
                        dm_embed.add_field(
                            name="🏅 Weekly Rank",
...
async def addpoints_command(interaction: discord.Interaction, user: discord.Member, points: int):
    ...
    await interaction.response.send_message(
        f"Added {points} points to {user.name}. New total: {current_user['points']}", ephemeral=True)
...
async def forcefetch_command(interaction: discord.Interaction):
    ...
    await interaction.followup.send(f"Found {len(upcoming)} matches. Posted {posted_count} new matches.",
                                    ephemeral=True)
...
async def repostmatches_command(interaction: discord.Interaction):
    ...
    for competition, comp_matches in matches_by_comp.items():
        ...
        for match in comp_matches:
            ...
            match_time = ensure_utc(match['match_time'])
            ...
            hours_to_vote = int(time_to_vote.total_seconds() // 3600)
            embed.set_footer(
                text=f"⏳ Voting closes 10 minutes before kickoff • You have ~{hours_to_vote} hours to vote!")
            ...
            try:
                ...
                separator_line = discord.Embed(description="───────────────────────────────",
...
async def upcoming_command(interaction: discord.Interaction, user: discord.Member = None):
    ...
    if not predictions:
        await interaction.followup.send("No upcoming or ongoing predictions.", ephemeral=True)
        return
    ...
    for pred in predictions:
        if pred['match_time']:
            match_time = ensure_utc(pred['match_time'])
...
async def mystats_command(interaction: discord.Interaction):
    ...
    with db_connection() as conn:
        ...
        cur.execute("""
                    ...
                    WHERE p.user_id = %s
                      AND pm.competition IS NOT NULL
...
async def compare_command(interaction: discord.Interaction, user: discord.Member):
    ...
    with db_connection() as conn:
        ...
        cur.execute("""
                    ...
                    WHERE p1.user_id = %s
                      ...
                      AND pm.status = 'FINISHED'
                      AND pm.home_score IS NOT NULL
                    ORDER BY pm.match_time DESC LIMIT 5
                    """, (user1_id, user2_id))
    ...
    if head_to_head:
        ...
        for match in head_to_head:
            ...
            h2h_text.append(
                f"{match['home_team']} {match['home_score']}-{match['away_score']} {match['away_team']}: {result}")
