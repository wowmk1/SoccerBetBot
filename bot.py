...
# ==== VOTE TRACKING ====
vote_data = {}  # match_id: {home, draw, away, votes_msg_id, locked_users, buttons_disabled}
...
async def generate_match_image(home_url, away_url):
    async with aiohttp.ClientSession() as session:
        home_bytes, away_bytes = None, None
        try:
            if home_url:
                async with session.get(home_url) as r: home_bytes = await r.read()
        except: pass
        try:
            if away_url:
                async with session.get(away_url) as r: away_bytes = await r.read()
        except: pass
...
class VoteButton(Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if votes_msg_id:
            msg = await interaction.channel.fetch_message(votes_msg_id)
            await msg.edit(embed=embed)
        else:
            msg = await interaction.channel.send(embed=embed)
            vote_data[match_id]["votes_msg_id"] = msg.id
        uid = str(user.id)
        if uid not in leaderboard:
            leaderboard[uid] = {"name":user.name,"points":0,"predictions":{}}
        leaderboard[uid]["predictions"][match_id] = self.category
        save_leaderboard()
...
async def post_match(match):
    ...
    if match_id in posted_matches: return
    match_time = datetime.fromisoformat(match['utcDate'].replace("Z","+00:00"))
    if match_time < datetime.now(timezone.utc): return
    ...
    if home_crest or away_crest:
        buf = await generate_match_image(home_crest, away_crest)
        file = discord.File(fp=buf, filename="match.png")
        embed.set_image(url="attachment://match.png")
    view = View()
    view.add_item(VoteButton("Home","home",match_id,match_time))
    view.add_item(VoteButton("Draw","draw",match_id,match_time))
    view.add_item(VoteButton("Away","away",match_id,match_time))
    msg = await channel.send(embed=embed,file=file,view=view)
    vote_data[match_id] = {"home":set(),"draw":set(),"away":set(),
                           "votes_msg_id":msg.id,"locked_users":set(),"buttons_disabled":False}
    posted_matches.add(match_id)
    save_posted()

# ==== UPDATE RESULTS + LEADERBOARD ====
                ...
                for m in data.get("matches",[]):
                    ...
                    if match_id in vote_data:
                        try:
                            ...
                            if msg_id:
                                channel = bot.get_channel(MATCH_CHANNEL_ID)
                                votes_msg = await channel.fetch_message(msg_id)
                                embed = create_votes_embed(match_id,match_result=result)
                                new_view = View()
                                for item in votes_msg.components[0].children:
                                    item.disabled=True
                                    new_view.add_item(item)
                                await votes_msg.edit(embed=embed,view=new_view)
                                vote_data[match_id]["buttons_disabled"]=True
                        except Exception as e: print("Failed updating votes:", e)
...
async def matches_command(interaction: discord.Interaction):
    ...
    if not matches:
        await interaction.response.send_message("No matches in next 24h", ephemeral=True)
        return
    league_dict={}
    for m in matches:
